function openModal(productId) {
  ...
  if (product) {
    ...
    document.getElementById("modal-image-2").src =
      product.image2 || product.image;
    document.getElementById("modal-image-3").src =
...
function limitQuantity(input, max) {
  if (input.value > max) {
    // Cette condition vérifie si la valeur actuelle dans l'input dépasse le maximum autorisé //
    input.value = max;
  }
  if (input.value < 1) {
    // Cette condition vérifie que "1" reste la valeur inférieur pour tour les modal //
    input.value = 1;
  }
}
...
function ajouterAuPanier(productId, quantite = 1) {
  // Cette fonction ajoute un produit au panier ou met à jour sa quantité s'il est déjà présent. //
  ...
  if (product && product.itemsRestants >= quantite) {
    ...
    if (itemExistant) {
      ...
      panier.push({
        id: productId,
        nom: product.nom,
        prix: product.prix, // Sinon, elle ajoute un nouvel élément au panier avec les détails du produit. //
        quantite: quantite,
      });
    }
    ...
    if (modal.style.display === "flex") {
      document.getElementById("modal-items-restants").textContent =
        product.itemsRestants; //Cette ligne assure que les informtions sur le stock restant aussi mis a jour sur les modal pour la quantité //
    ...
    if (productElement) {
      const itemsRestantsElement =
        productElement.querySelector(".items-restants");
...
function mettreAJourBadgePanier() {
  // Elle calcule le nombre total d'articles dans le panier (). //
...
function disableScroll() {
  // Fonction pour désctivé le scroll officiellement définit //
...
document.addEventListener("DOMContentLoaded", () => {
  // La fonction crée car pour recrée les carte div a partir de l'array Produit //
  ...
  function afficherCartes(categorie, containerId) {
    //  La fonction crée car pour afficher les carte div a partir de l'array Produit si le container inner HTML est vide. //
  ...
  function gererBarreRecherche(frame) {
    ...
    if (frame === "accueil") {
      // Mettre la barre recherche et le rechercher en disebled au frame "Accueil" //
  ...
  function currentFrame(frame) {
    // La fonction prend un paramètre frame qui représente la section à afficher. //
    ...
    if (frame === "arbres") {
      ...
      document
        .querySelector("#Materiel .next-button")
        .classList.add("disabled"); // Pour la section accueil, elle affiche la section accueil et sont contenue au complet. //
    } else {
...
document.addEventListener("contextmenu", function (event) {
  ...
  if (carteItem) {
    // Si le clic droit est sur un élément carte-item, empêcher le menu contextuel par défaut
...
document.addEventListener("click", function () {
  // Masquer le menu contextuel personnalisé lors d'un clic gauche //
  const menu = document.querySelector(".menu-contextuel");
  menu.style.display = "none";
});
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if (selectedOption === "Ajouter au panier") {
      // Selon les option selectioné dans le menu contextuel //
...
document
  ...
  .addEventListener("click", function () {
    ...
    for (let categorie in produits) {
      const produitTrouve = produits[categorie].find(
        (p) => p.nom === modalName
      );
      if (produitTrouve) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.addEventListener("click", function (event) {
    if (event.target == modal) {
      // Fermeture du modal en cliquant si on clique dehors de celui-ci pour chacune des pages, ce addEventListener est la 3 fois dans le code //
      closeModalFunction();
    }
  });

  document.addEventListener("click", function (event) {
    // Ouverture du modal si on click avec la fleche gauche de la souris //
    const carteItem = event.target.closest(".carte-item");
    if (carteItem) {
      const productId = carteItem.id;
      openModal(productId);
